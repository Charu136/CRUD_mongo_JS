...
print('Result after Updating the task fields - Removing Due on and adding completed status');
while(result.hasNext()){
printjson(result.next());
}

//Sorting the username of the members of a project in alphabetical order 
db.projects.update({project: "P1"}, {$push: {user: {$sort: {userName:1}}}});
result = db.projects.find({project: "P1"});
print('Result after Sorting the username of the members of a project in alphabetical order ');
while(result.hasNext()){
printjson(result.next());
}
...
result = db.projects.find({project: "P1"});
print('Result after Removing the first user from the sorted user list');
while(result.hasNext()){
printjson(result.next());
}
...
print('Result after Upserting sections to a project');
while(result.hasNext()){
printjson(result.next());
}
